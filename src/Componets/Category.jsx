    ...
    return (
        <>
            <div className='w-full flex justify-center items-center mt-2'>
                <p className='w-[80%] flex bg-white font-semibold pl-[10%] text-2xl pt-4'>Select Category</p>
            </div>
            <div className='w-full flex justify-center items-center'>
                <div className='w-[80%] bg-white flex justify-center items-center'>
                    <div className='bg-white flex py-4 gap-[3rem] overflow-hidden overflow-x-scroll scroll ml-[1rem]'>
                        {
                            category.map((item, index) => {
                                return <div className={`flex flex-col justify-center items-center bg-white cursor-pointer`} key={index} onClick={()=>setcategory_filter(prev=>prev===item.name?"All":item.name)}>
                                    ...
                                    <p className='font-bold text-[#333] bg-white'>{item.name}</p>
                                </div>
                            })
                        }
                    </div>
